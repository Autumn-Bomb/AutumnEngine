...
void AutumnEngine::GUI::CreateSpriteUIElement(const std::string textureName, const sf::Vector2f position, const sf::Vector2f size, const sf::Color color, const int layer)
{
	...
	AutumnEngine::Sprite t_Sprite = AutumnEngine::Sprite(t_Texture, position, size, layer);
	t_Sprite.SetColour(color);
